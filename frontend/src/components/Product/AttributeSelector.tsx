...
const AttributeSelector: React.FC<AttributeSelectorProps> = ({ 
  ...
  const isColorAttribute = attribute.name.toLowerCase() === 'color' || attribute.type === 'swatch';
        ...
        {attribute.items?.map((option) => {
          ...
          return (
            <button
              ...
              `}
              style={isColorAttribute ? { backgroundColor: option.value } : {}}
              title={isColorAttribute ? option.displayValue : undefined}
              disabled={disabled}
            >
              {!isColorAttribute && option.displayValue}
            </button>
          );
        })}
      </div>
